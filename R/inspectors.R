...
inspector.CAPTURES <- function(x, ...){
  ...
  v16 = interval_validator( x[species == "REPH", .(gps_id)],      v = data.table(variable = "gps_id",    lq = 1, uq = 13),         "GPS ID not in use" )
  ...
  v19 = combo_validator(x[, .( UL, LL, UR, LR, recapture)],
  ...
  v20  = combo_validator(x[, .( UL, LL, UR, LR, recapture = 1)], validSet = colorCombos() )
...
inspector.RESIGHTINGS <- function(x, ...){
  ...
  v20 = combo_validator(x[, .( UL, LL, UR, LR, recapture = 1)],
                        validSet  = c(idbq('select CONCAT(UL, "-", LL, "|",UR, "-", LR) combo FROM CAPTURES' )$combo,
                                      idbq('select CONCAT(UL, "-", LL, "|",UR, "-", LR) combo FROM FIELD_2017_REPHatBARROW.CAPTURES' )$combo),
...
inspector.NESTS <- function(x, ...){
  ...
  possible_nests = rbind(paste0("R", 101:999), paste0("N", 101:999), paste0("P", 101:999), paste0("S", 101:999))
